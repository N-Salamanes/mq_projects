  ...
  def translateStmt(stmt : Statement) : J.Statement =
    stmt match {
      ...
      case ForStmt(i, s, e, st, b) =>
        // re-usable values
        val IdnDef_idn = J.IdnDef(getTargetName(i))
        val IdnDef_end = J.IdnDef(getTargetName(b, "_end"))
        val IdnDef_step = J.IdnDef(getTargetName(b, "_step"))
        val IdnDef_dirn = J.IdnDef(getTargetName(b, "_dirn"))
      ...
      case s @ IterateOverStmt(i, ns, b) =>
        // Enclose in a block.
        J.Block(
          // declare a new identifier of type cell, also get uniqualified name of name type.  E.g: current, nbr, etc.
